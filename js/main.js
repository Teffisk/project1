document.addEventListener("DOMContentLoaded", function(e) {
  ...
  function addStartSplash() {
    ...
    // Add some text
    startSplash.appendChild(
      document.createTextNode(
        "Welcome to Indy's Taco Truck. In order to run a successful food truck, you'll need to buy all of your ingredients to make yummy tacos for people to enjoy. The more tacos you sell, the more money you'll make and the better reputation you'll have. Make sure to stock up on ingredients before customers show up. Make sure to keep tortillas and salsa in stock or you can't make any tacos! Click here to begin!"
      )
    );
  ...
  function startNewGame() {
    ...
    totalOrders = chicken.orders + steak.orders + avocado.orders;
    ...
    adjustChanceToBuy();
    console.log("chicken.price", displayChickenQty);
    updateTheDom();
    ...
    if (truckOpen == false) {
      ...
      if (newPasserby.behavior == "customer") {
        ...
        if (x < 0.4) {
          ...
          if (chicken.inv <= 0 || salsa.inv <= 0 || tortillas.inv <= 0) {
          ...
          if (avocado.inv <= 0 || salsa.inv <= 0 || tortillas.inv <= 0) {
            console.log(
              "Aw, bummer, you are out of avocado tacos. I'm leaving"
            );
          ...
          if (steak.inv <= 0 || salsa.inv <= 0 || tortillas.inv <= 0) {
  ...
  function runImageCommands(passerby) {
    ...
    if (passerby.behavior == "customer" && passerby.direction == "right") {
      ...
      passerby.behavior == "customer" &&
      passerby.direction == "left"
    ) {
      passerby.image = document.createElement("img");
      ...
      passerby.behavior == "noncustomer" &&
      passerby.direction == "right"
    ) {
      passerby.image = document.createElement("img");
      passerby.image.src = "./img/huskieRight.gif";
      passerby.image.setAttribute("class", "customer");
  ...
  var left = 0,
    game = document.getElementById("taco-truck-scene");
  ...
  function buyATacoRight(currentCustomer) {
    ...
    if (
      currentCustomer.speech == "orderChicken" ||
      currentCustomer.speech == "orderSteak" ||
      currentCustomer.speech == "orderAvocado"
    ) {
      setTimeout(function() {
        walkAwayWithTacoRight(currentCustomer);
      }, 2000);
    } else {
      setTimeout(function() {
        walkAwayWithoutTacoRight(currentCustomer);
      }, 2000);
    }
  }
  ...
  function buyATacoLeft(currentCustomer) {
    ...
    if (
      ...
      setTimeout(function() {
        walkAwayWithTacoLeft(currentCustomer);
      }, 2000);
    } else {
      setTimeout(function() {
        walkAwayWithoutTacoLeft(currentCustomer);
      }, 2000);
    }
  }
  ...
  function deleteCustomer(currentCustomer) {
    ...
    //passersbyArray.pop();
    document
      .querySelector("#taco-truck-scene")
      .removeChild(currentCustomer.image);
  }
  ...
  function endOfDay() {
    ...
    if (score <= 0) {
      ...
      eodSplash.appendChild(
        document.createTextNode(
          `Yikes, your business is on the rails! See if you can do better tomorrow! Click here to start the next day.     Money in the account: $${account -
            startingAccount}. Reputation: ${reputation.toFixed(
            1
          )}. Customer Conversion Rate: ${(conversion * 100).toFixed(
            1
          )}%. Score: ${score}.`
        )
      );
      ...
      eodSplash.appendChild(
        document.createTextNode(
          `Congratulations, you survived the day! See if you can do even better tomorrow! Click here to start the next day.     Money in the account: $${account -
            startingAccount}. Reputation: ${reputation.toFixed(
            1
          )}. Customer Conversion Rate: ${(conversion * 100).toFixed(
            1
      ...
      eodSplash.appendChild(
        document.createTextNode(
          `Hey, that's pretty great! You are running a successful business. See if you can do even better tomorrow! Click here to start the next day.     Money in the account: $${account -
            startingAccount}. Reputation: ${reputation.toFixed(
            1
          )}. Customer Conversion Rate: ${(conversion * 100).toFixed(
            1
      ...
      eodSplash.appendChild(
        document.createTextNode(
          `Wow, killer score! See if you can do even better tomorrow! Click here to start the next day.     Money in the account: $${account -
            startingAccount}. Reputation: ${reputation.toFixed(
            1
          )}. Customer Conversion Rate: ${(conversion * 100).toFixed(
            1
  ...
  function checkInv() {
    if (
      ...
      console.log(
        "Uh oh, you don't have enough ingredients to make more tacos. You need to buy more inventory."
      );
    }
  }

  function placeChickenOrder() {
    account = account + chicken.price;
    chicken.inv--;
    salsa.inv--;
    tortillas.inv--;
    chicken.orders++;
